...
function portalWindowRuntimeEnabled() {
	return !!graphicsSettings.portalWindow;
}

function applyPortalWindowMaterialContract(plane) {
	if (!plane || !plane.material) return;
	const enabled = portalWindowRuntimeEnabled();
	plane.material.map = enabled ? mirrorRenderTarget.texture : null;
	plane.material.color.setHex(enabled ? 0xffffff : 0x0d1820);
	plane.material.transparent = true;
	plane.material.opacity = enabled ? 1.0 : 0.42;
...
function renderControlsHud() {
	...
	controlsBody.innerHTML = `
		<div class="control-section">
			...
			<div class="control-section-text">Break humans into soul cubes. Vacuum them. Shoot stored souls into the air goals. Fill every goal to open the mirror door.</div>
		</div>
		<div class="control-section">
...
const ROOM_CONTINUATION_OFFSETS = [-ROOM_DEPTH, ROOM_DEPTH];
...
const roomContinuationGridCells = ROOM_CONTINUATION_OFFSETS.map(() => []);
const roomContinuationGridSoulCubes = ROOM_CONTINUATION_OFFSETS.map(() => []);
const mirrorRenderTarget = new THREE.WebGLRenderTarget(1024, 1024, { depthBuffer: true, stencilBuffer: false });
...
const mirrorCamera = new THREE.PerspectiveCamera(70, 1, 0.1, 120);
const mirrorPlaneNormal = new THREE.Vector3(0, 0, 1);
const mirrorCameraOffset = new THREE.Vector3();
...
const room = {
	...
	depositCooldown: 0,
	doorLoopCooldown: 0,
	lastDoorCheckZ: ROOM_START_Z
};

const doorCrossingState = {
	// Gameplay/topology state only. This is not a camera transition owner.
	active: false,
	mode: "loop",
	direction: "sourceToDestination",
	time: 0,
	playerPosAtCross: new THREE.Vector3(),
	playerVelAtCross: new THREE.Vector3(),
	yawAtCross: 0,
	pitchAtCross: 0
};
...
const portalMappedCameraPosition = new THREE.Vector3();
const portalMappedPlayerPosition = new THREE.Vector3();
const portalMappedVelocity = new THREE.Vector3();
const portalCrossingCameraDelta = new THREE.Vector3();
const portalVirtualEye = new THREE.Vector3();
const portalVirtualLook = new THREE.Vector3();
const portalVirtualAim = new THREE.Vector3();
const portalCameraCompression = {
	value: 0,
	target: 0,
	startDistance: 6.2,
	fullDistance: 1.15,
...
function getPortalCameraCompressionTarget() {
	return 0;
}

function updatePortalCameraCompression() {
	portalCameraCompression.target = 0;
	portalCameraCompression.value = 0;
	return 0;
}

function mapSameRoomPortalPosition(sourcePosition, target = new THREE.Vector3()) {
	// Portal-view mapping for a camera still inside the source side of the room.
	// It answers: “where would this eye be if it were looking out from the
	// room-start portal instead of standing in front of the exit portal?”
	const distanceFromDoorInside = THREE.MathUtils.clamp(
		sourcePosition.z - fixedPortal.sourceZ,
		fixedPortal.minCameraDistance,
		fixedPortal.maxCameraDistance
	);
	target.set(
		THREE.MathUtils.clamp(sourcePosition.x, -ROOM_WIDTH * 0.43, ROOM_WIDTH * 0.43),
		THREE.MathUtils.clamp(sourcePosition.y, GROUND_Y + 0.50, ROOM_WALL_HEIGHT - 0.20),
		fixedPortal.destinationZ - distanceFromDoorInside
	);
	return target;
}
...
function mapSameRoomPortalCrossingPosition(sourcePosition, target = new THREE.Vector3(), direction = "sourceToDestination") {
	// Door topology mapping: preserve local doorway coordinates. A slow walk through
	// the threshold should keep the same sideways offset, height, look direction,
	// and through-plane residual instead of snapping to a destination center.
	const localX = THREE.MathUtils.clamp(sourcePosition.x - roomExitPos.x, -fixedPortal.halfWidth, fixedPortal.halfWidth);
	const localY = THREE.MathUtils.clamp(sourcePosition.y, fixedPortal.bottomY, fixedPortal.topY);
	if (direction === "destinationToSource") {
		const distanceThroughDoor = Math.max(fixedPortal.seamEpsilon, sourcePosition.z - fixedPortal.destinationZ);
		target.set(roomExitPos.x + localX, localY, fixedPortal.sourceZ + distanceThroughDoor);
	} else {
		const distanceThroughDoor = Math.max(fixedPortal.seamEpsilon, fixedPortal.sourceZ - sourcePosition.z);
		target.set(roomExitPos.x + localX, localY, fixedPortal.destinationZ - distanceThroughDoor);
	}
	return target;
}

function mapSameRoomPortalVelocity(sourceVelocity, target = new THREE.Vector3(), direction = "sourceToDestination") {
	target.copy(sourceVelocity);
	if (direction === "destinationToSource") {
		if (Math.abs(target.z) < 0.35) target.z = 0.35;
		if (target.z < 0) target.z = -target.z;
	} else {
		if (Math.abs(target.z) < 0.35) target.z = -0.35;
		if (target.z > 0) target.z = -target.z;
	}
...
function getActiveHumanTarget() {
	return Math.min(TARGET_COUNT, ACTIVE_HUMAN_TARGET_CAP, ACTIVE_HUMAN_TARGET + Math.max(0, room.index - 1));
}
...
const shotClosestPos = new THREE.Vector3();
...
function clearRoomEnvironment() {
	clearThreeGroup(roomGroup, { preservedTextures: new Set([mirrorRenderTarget.texture]) });
	roomGridCells.length = 0;
	roomGridSoulCubes.length = 0;
	for (let i = 0; i < ROOM_CONTINUATION_OFFSETS.length; i++) {
...
function addRoomBox(width, height, depth, x, y, z, material) {
 ...
 *    locked source door crossing      → remap player to destination doorway
 *    locked destination door crossing → remap player back to source doorway
...
function buildRoomEnvironment() {
	...
	const mirrorMat = new THREE.MeshBasicMaterial({ map: mirrorRenderTarget.texture, color: 0xffffff, side: THREE.DoubleSide });
	const mirrorFrameMat = new THREE.MeshStandardMaterial({ color: 0xd8dde0, emissive: 0x101820, emissiveIntensity: 0.0, roughness: 0.18, metalness: 0.92 });
	...
	function addVisualContinuationShell(offsetZ) {
		const copyFloor = addRoomBox(ROOM_WIDTH, 0.08, ROOM_DEPTH, 0, LAB_FLOOR_TOP_Y - 0.04, offsetZ, floorMat.clone());
		copyFloor.castShadow = false;
		copyFloor.receiveShadow = true;
		const copyCeiling = addRoomBox(ROOM_WIDTH, 0.16, ROOM_DEPTH, 0, ROOM_WALL_HEIGHT + 0.08, offsetZ, wallMat.clone());
		...
		addRoomBox(sideWallWidth, ROOM_WALL_HEIGHT, 0.5, -sideWallX, ROOM_WALL_HEIGHT / 2, offsetZ + ROOM_DEPTH / 2, copyWallMat);
		addRoomBox(sideWallWidth, ROOM_WALL_HEIGHT, 0.5, sideWallX, ROOM_WALL_HEIGHT / 2, offsetZ + ROOM_DEPTH / 2, copyWallMat);
		addRoomBox(doorOpeningWidth, topWallHeight, 0.5, 0, topWallY, offsetZ + ROOM_DEPTH / 2, copyWallMat);
		addRoomBox(sideWallWidth, ROOM_WALL_HEIGHT, 0.5, -sideWallX, ROOM_WALL_HEIGHT / 2, offsetZ - ROOM_DEPTH / 2, copyWallMat);
		addRoomBox(sideWallWidth, ROOM_WALL_HEIGHT, 0.5, sideWallX, ROOM_WALL_HEIGHT / 2, offsetZ - ROOM_DEPTH / 2, copyWallMat);
		addRoomBox(doorOpeningWidth, topWallHeight, 0.5, 0, topWallY, offsetZ - ROOM_DEPTH / 2, copyWallMat);
		addRoomBox(0.5, ROOM_WALL_HEIGHT, ROOM_DEPTH, -ROOM_WIDTH / 2, ROOM_WALL_HEIGHT / 2, offsetZ, copyWallMat);
		addRoomBox(0.5, ROOM_WALL_HEIGHT, ROOM_DEPTH, ROOM_WIDTH / 2, ROOM_WALL_HEIGHT / 2, offsetZ, copyWallMat);
	}
	for (const offsetZ of ROOM_CONTINUATION_OFFSETS) addVisualContinuationShell(offsetZ);
	roomDoorMesh = null;
	roomMirrorPlane = null;
	roomMirrorFrame = null;
	roomEntranceMirrorPlane = null;
	...
	for (let i = 0; i < room.requiredSouls; i++) {
		...
		for (let c = 0; c < ROOM_CONTINUATION_OFFSETS.length; c++) {
			const offsetZ = ROOM_CONTINUATION_OFFSETS[c];
			const copyFrame = new THREE.Mesh(new THREE.BoxGeometry(0.72, 0.72, 0.06), goalFrameMat.clone());
			copyFrame.position.set(x, ROOM_GOAL_Y, ROOM_GRID_Z + offsetZ - 0.04);
			copyFrame.castShadow = false;
			copyFrame.receiveShadow = false;
			roomGroup.add(copyFrame);

			const copyHole = new THREE.Mesh(new THREE.BoxGeometry(0.52, 0.52, 0.08), goalHoleMat.clone());
			copyHole.position.set(x, ROOM_GOAL_Y, ROOM_GRID_Z + offsetZ);
			copyHole.castShadow = false;
			copyHole.receiveShadow = false;
			roomGroup.add(copyHole);

			const copyCube = new THREE.Mesh(new THREE.BoxGeometry(0.36, 0.36, 0.36), depositedSoulMat.clone());
			copyCube.position.set(x, -999, ROOM_GRID_Z + offsetZ + 0.12);
			copyCube.castShadow = false;
			copyCube.receiveShadow = true;
			roomGroup.add(copyCube);
			roomContinuationGridCells[c].push({ rim: copyFrame, hole: copyHole, x, y: ROOM_GOAL_Y, z: ROOM_GRID_Z + offsetZ });
	...
	for (let i = 0; i < obstacleCount; i++) {
		...
		for (const offsetZ of ROOM_CONTINUATION_OFFSETS) {
			const copy = addRoomBox(w, h, d, px, h / 2, pz + offsetZ, obstacleMat.clone());
...
function resolvePlayerObstacleCollisions() {
	...
	for (const c of roomColliders) {
		...
		if (
			...
			player.pos.z > c.minZ - radius && player.pos.z < c.maxZ + radius
		) {
			const pushLeft = Math.abs(player.pos.x - (c.minX - radius));
			const pushRight = Math.abs((c.maxX + radius) - player.pos.x);
			const pushBack = Math.abs(player.pos.z - (c.minZ - radius));
			const pushForward = Math.abs((c.maxZ + radius) - player.pos.z);
			...
			else if (minPush === pushBack) { player.pos.z = c.minZ - radius; if (player.vel.z > 0) player.vel.z = 0; }
			else { player.pos.z = c.maxZ + radius; if (player.vel.z < 0) player.vel.z = 0; }
		}
	}
}
...
function getPlayerWallContact() {
	...
	if (player.pos.z <= minZ + climbGap) return { x: 0, z: 1, topY: getPlayerCeilingLimit() };
	if (player.pos.z >= maxZ - climbGap) return { x: 0, z: -1, topY: getPlayerCeilingLimit() };

	for (const c of roomColliders) {
		if (player.pos.y < c.bottomY - 0.2 || player.pos.y > c.topY + 1.1) continue;
		const inZ = player.pos.z > c.minZ - radius && player.pos.z < c.maxZ + radius;
		...
		if (inX && Math.abs(player.pos.z - (c.minZ - radius)) < climbGap + 0.05) return { x: 0, z: -1, topY: c.topY };
		if (inX && Math.abs(player.pos.z - (c.maxZ + radius)) < climbGap + 0.05) return { x: 0, z: 1, topY: c.topY };
	}
	return null;
}
...
function depositShotSoulToRoom(slot) {
	...
	spawnParticleBurst(targetData[idx + T_X], targetData[idx + T_Y], targetData[idx + T_Z]);
	const filledCell = roomGridCells[fillIndex];
	spawnParticleBurst(filledCell ? filledCell.x : roomDepositPos.x, filledCell ? filledCell.y : roomDepositPos.y, filledCell ? filledCell.z : roomDepositPos.z);
...
function setDoorCrossingMaskActive(active) {
	// The old transition mask remains in the DOM for CSS compatibility, but the
	// current door contract is immediate topology, not a visual fade/transition.
	if (!doorCrossingMask) return;
	doorCrossingMask.classList.toggle("active", false);
}

function captureDoorCrossingState(mode, direction = "sourceToDestination") {
	doorCrossingState.mode = mode;
	doorCrossingState.direction = direction;
	doorCrossingState.time = 0;
	doorCrossingState.playerPosAtCross.copy(player.pos);
	doorCrossingState.playerVelAtCross.copy(player.vel);
	doorCrossingState.yawAtCross = cursor.yaw;
	doorCrossingState.pitchAtCross = cursor.pitch;
}

function resolveDoorCrossingAction(mode, direction = "sourceToDestination") {
	// Door crossing dispatcher: gameplay/topology only. Portal-window rendering is
	// handled separately by renderMirrorDoor().
	captureDoorCrossingState(mode, direction);
	if (mode === "advance") {
		enterNextRoomThroughPortal(direction);
	} else {
		loopPlayerThroughClosedDoor(direction);
	}
}

function finishDoorCrossing() {
	doorCrossingState.active = false;
	setDoorCrossingMaskActive(false);
}

function updateDoorCrossingState(dt) {
	// Kept as a safe no-op guard for older transition-style frame paths.
	doorCrossingState.active = false;
}

function applyPortalBodyHandoff(targetRoomAdvanced = false) {
	// Portal-body remapping is disabled in the simple-door build.
	// Keep this function as a compatibility stub for older call sites.
	if (!targetRoomAdvanced) loopPlayerThroughClosedDoor();
}
...
function enterNextRoomThroughPortal(direction = "sourceToDestination") {
	// Open-door contract: advancing rooms should still feel like walking through the
	// same doorway, so preserve seam-relative body state before rebuilding room data.
	mapSameRoomPortalCrossingPosition(player.pos, portalMappedPlayerPosition, direction);
	mapSameRoomPortalVelocity(player.vel, portalMappedVelocity, direction);
	portalCrossingCameraDelta.subVectors(portalMappedPlayerPosition, player.pos);
	room.index++;
	room.seed = Math.floor(Math.random() * 1000000) + room.index * 9973;
	resetRoomInventory(0.45);
	player.pos.copy(portalMappedPlayerPosition);
	player.vel.copy(portalMappedVelocity);
	player.jumpVel = player.vel.y;
	camera.position.add(portalCrossingCameraDelta);
	room.lastDoorCheckZ = player.pos.z;
	room.doorLoopCooldown = 0.035;
	doorCrossingState.active = false;
	portalCameraCompression.value = 0;
	portalCameraCompression.target = 0;
...
function loopPlayerThroughClosedDoor(direction = "sourceToDestination") {
	// Locked-door contract: the room does not reset and the room index does not
	// change. Only the player body crosses the door topology; the camera remains a
	// normal follower and the door window remains visual-only.
	mapSameRoomPortalCrossingPosition(player.pos, portalMappedPlayerPosition, direction);
	mapSameRoomPortalVelocity(player.vel, portalMappedVelocity, direction);
	portalCrossingCameraDelta.subVectors(portalMappedPlayerPosition, player.pos);
	player.pos.copy(portalMappedPlayerPosition);
...
window.graphicsDebug = function graphicsDebug() { return { ...graphicsSettings, legacyMode: isLegacyGraphicsMode(), particlesRuntimeEnabled: particlesRuntimeEnabled(), portalWindowRuntimeEnabled: portalWindowRuntimeEnabled(), presetContract: getGraphicsPreset(graphicsSettings.preset), pixelRatio: renderer.getPixelRatio(), shadowsEnabled: renderer.shadowMap.enabled, particleMeshVisible: !!(particleMesh && particleMesh.visible) }; };
window.portalDoorDebug = function portalDoorDebug() {
	return {
		...
		lastDoorCheckZ: room.lastDoorCheckZ,
		...
		cooldown: room.doorLoopCooldown
	};
};

function tryResolveDoorCrossing() {
	// Continuous wrapped-space owner. No crossing event, no cooldown gate.
	const withinDoorWidth = Math.abs(player.pos.x - roomExitPos.x) < fixedPortal.halfWidth;
	const withinDoorAperture = withinDoorWidth && isInsideDoorAperture(player.pos, 0.04);
	if (!withinDoorAperture) return false;
	if (!room.doorOpen) {
		const crossedSourcePlane = room.lastDoorCheckZ > fixedPortal.sourceZ && player.pos.z <= fixedPortal.sourceZ;
		if (crossedSourcePlane) {
			resolveDoorCrossingAction('loop', 'sourceToDestination');
			return true;
		}
		const crossedDestinationPlane = room.lastDoorCheckZ < fixedPortal.destinationZ && player.pos.z >= fixedPortal.destinationZ;
		if (crossedDestinationPlane) {
			resolveDoorCrossingAction('loop', 'destinationToSource');
			return true;
		}
	} else {
		const crossedSourcePlane = room.lastDoorCheckZ > fixedPortal.sourceZ && player.pos.z <= fixedPortal.sourceZ;
		if (crossedSourcePlane) { resolveDoorCrossingAction('advance','sourceToDestination'); return true; }
	}
	return false;
}
...
function updateRoomLoop(dt) {
	...
	room.doorLoopCooldown = Math.max(0, room.doorLoopCooldown - dt);
	tryResolveDoorCrossing();
	room.lastDoorCheckZ = player.pos.z;
	updateRoomVisuals();
}
...
function resolveSeparatedDropPosition(x, z) {
	const baseAngle = Math.atan2(z - player.pos.z, x - player.pos.x);
...
function spawnFlowerPowerup(x, y, z) {
	...
	mesh.visible = true;
	const continuationMeshes = ROOM_CONTINUATION_OFFSETS.map(() => acquireFlowerPowerupMirrorMesh());
	...
	for (let i = flowerPowerups.length - 1; i >= 0; i--) {
		...
		if (powerup.continuationMeshes) {
			for (let c = 0; c < powerup.continuationMeshes.length; c++) {
				...
				continuationMesh.position.set(powerup.x, powerup.mesh ? powerup.mesh.position.y : powerup.y, powerup.z + ROOM_CONTINUATION_OFFSETS[c]);
		...
		const dz = powerup.z - player.pos.z;
		...
		if (dx * dx + dz * dz + dy * dy <= FLOWER_PICKUP_RADIUS * FLOWER_PICKUP_RADIUS) {
			...
			spawnParticleBurst(powerup.x, powerup.y + 0.2, powerup.z);
			releaseFlowerPowerupMesh(powerup.mesh);
			if (powerup.continuationMeshes) {
				for (const mesh of powerup.continuationMeshes) releaseFlowerPowerupMirrorMesh(mesh);
			}
...
const CONTINUATION_INSTANCE_COUNT = TARGET_COUNT * ROOM_CONTINUATION_OFFSETS.length;
const continuationTargetMesh = new THREE.InstancedMesh(targetGeometry, targetMaterial.clone(), CONTINUATION_INSTANCE_COUNT);
continuationTargetMesh.castShadow = false;
...
function updateHumanModelInstance(i, visible, x, z, yaw, scale, dt, attackTimer = 0, attackVariant = 0) {
	const root = HUMAN_MODEL_ROOTS[i];
	const mixer = HUMAN_MODEL_MIXERS[i];
	const bones = HUMAN_MODEL_POSE_BONES[i];
...
function keepSoulCoreOutsidePhoneSolid(idx, preferScreenFront = false) {
	...
	targetData[idx + T_Z] = phoneSolidWorld.z;
	targetData[idx + T_VX] = 0;
	targetData[idx + T_VY] = 0;
	targetData[idx + T_VZ] = 0;
	return true;
...
function damageSoulShell(i, amount, hitX, hitY, hitZ) {
	...
	const awayZ = targetData[idx + T_Z] - player.pos.z;
...
function applyProximityAttackHits() {
	...
	for (let i = 0; i < TARGET_COUNT; i++) {
		...
		const dz = targetData[idx + T_Z] - player.pos.z;
		...
		const hitZ = targetData[idx + T_Z];
		proximityAttackHitTargets.add(i);
		proximityAttack.visualHit = true;
		proximityAttackImpact.set(hitX, hitY, hitZ);
		const hitDamage = (combo.damage ?? PROX_ATTACK_DAMAGE) * (1 + Math.min(0.75, hitCount * 0.12));
		if (damageSoulShell(i, hitDamage, hitX, hitY, hitZ)) {
			hitCount++;
			hitPoints.push({ x: hitX, y: hitY, z: hitZ });
			const flameStrength = hitCount > 1 ? 0.95 + hitCount * 0.18 : 0.55;
			spawnFlameBurst(hitX, hitY, hitZ, flameStrength);
			didHit = true;
		}
	}
	if (didHit) {
...
function chooseHumanSpawnPoint(i, avoidPoint = null) {
	...
	for (let tries = 0; tries < 32; tries++) {
		...
		const dzPlayer = candidate.z - player.pos.z;
...
function updateCamera() {
	const portalCompress = updatePortalCameraCompression();
	const thirdPersonDistance = THREE.MathUtils.lerp(3, 0.62, portalCompress);
	const thirdPersonHeight = THREE.MathUtils.lerp(1.1, 0.78, portalCompress);
	const lookLift = THREE.MathUtils.lerp(0.45, 0.18, portalCompress);
...
function updateVacuum(dt) {
	...
	if (attractionActive) {
		for (let i = 0; i < TARGET_COUNT; i++) {
			...
			if (!isSoulInAttractionOffer(x, y, z)) continue;
			soulPoint.set(x, y, z);
			soulToScreen.copy(vacuumPullPoint).sub(soulPoint);
			const distanceToScreen = soulToScreen.length();
			const cylinderBonus = isSoulInsideScreenCaptureCylinder(x, y, z) ? -3.5 : 0;
	...
	for (let i = 0; i < TARGET_COUNT; i++) {
		...
		const insideCylinder = isSoulInsideScreenCaptureCylinder(x, y, z);
		...
		if (soulState[i] === SOUL_RECOILING) {
			...
			if (isDepositShot) {
				...
				if (Math.random() < 0.22) spawnParticleBurst(targetData[idx + T_X], targetData[idx + T_Y], targetData[idx + T_Z]);
				if (hitHuman) {
					soulDepositShot[i] = 0;
					clearTargetSlot(i);
					continue;
		...
		if (soulState[i] === SOUL_LATCHED || soulState[i] === SOUL_INGESTING) {
			...
			if (sealDist > 0.001) {
				...
				targetData[idx + T_Z] += toSeal.z * sealMove;
...
function placePlayerAtRoomStart() {
	...
	player.jumpBufferTimer = 0;
	player.airJumpsRemaining = phoneCharacter.jump.airJumps;
	room.lastDoorCheckZ = ROOM_START_Z;
}
...
function constrainThirdPersonCamera(desired, lookBase) {
	...
	let nearestT = 1;
	for (const c of roomColliders) {
		const t = getSegmentAabbHitT(cameraCollisionStart, cameraCollisionEnd, c);
	...
	for (const c of roomBounds) {
		const t = getSegmentAabbHitT(cameraCollisionStart, cameraCollisionEnd, c, CAMERA_COLLISION_RADIUS * 0.8);
		if (t !== null && t < nearestT) nearestT = t;
	}
	if (nearestT < 1) {
	...
	desired.z = THREE.MathUtils.clamp(desired.z, -ROOM_DEPTH / 2 + CAMERA_COLLISION_RADIUS, ROOM_DEPTH / 2 - CAMERA_COLLISION_RADIUS);
...
function updatePlayerPhysics(dt) {
	...
	const crossedPortalDuringMove = tryResolveDoorCrossing();
	clampPlayerToRoomHeight();
	...
	if (player.pos.z < minZ) {
		...
		player.vel.x *= PARKOUR.wallSlideRetention;
	} else if (player.pos.z > maxZ) {
		player.pos.z = maxZ;
		if (player.vel.z > 0) player.vel.z = 0;
		player.vel.x *= PARKOUR.wallSlideRetention;
	}
...
function updateTargets() {
	...
	for (let i = 0; i < TARGET_COUNT; i++) {
		...
		if (humanVisible) {
			const modelVisible = updateHumanModelInstance(i, true, x, z, targetData[idx + T_WALK_YAW], livingScale, frame.dt, humanAlive ? targetData[idx + T_ATTACK_TIMER] : 0, targetData[idx + T_ATTACK_VARIANT]);
			if (modelVisible) {
				sprite.visible = false;
			} else {
				...
				sprite.position.set(x, HUMAN_GROUND_Y + 0.55 * scale, z);
				sprite.scale.set(0.82 * livingScale * faceSign, 1.18 * livingScale, 1);
				sprite.visible = true;
			}
		} else {
			updateHumanModelInstance(i, false, x, z, targetData[idx + T_WALK_YAW], 0, frame.dt);
			sprite.visible = false;
		}
		const humanScale = 0;

		springNetCenter.set(x, y, z);
		...
		for (let c = 0; c < ROOM_CONTINUATION_OFFSETS.length; c++) {
			const continuationInstance = i * ROOM_CONTINUATION_OFFSETS.length + c;
			const offsetZ = ROOM_CONTINUATION_OFFSETS[c];
			if (cubeMorph > 0.001) {
				continuationMatrix.copy(dummy.matrix);
				continuationMatrix.elements[14] += offsetZ;
		...
		for (let n = 0; n < LATTICE_NODE_COUNT; n++) {
			...
			if (ingest > 0.001 && livePinForTarget) {
				...
				const worldDesiredZ = z + springNetDesired.z;
		...
		for (let r = 0; r < LATTICE_RODS_PER_TARGET; r++) {
			...
			springNetA.set(
				...
				z + latticePos[aBase + 2]
			);
			springNetB.set(
				x + latticePos[bBase],
				y + latticePos[bBase + 1],
				z + latticePos[bBase + 2]
			);
...
function updateMirrorDoorCamera() {
	if (!roomMirrorPlane) return;
	// Window contract: render from the actual viewer camera mapped through the
	// doorway, not from a special transition camera. This makes the door surface
	// behave like a window into the same room.
	mapSameRoomPortalPosition(camera.position, portalMappedCameraPosition);
	mirrorCamera.position.copy(portalMappedCameraPosition);
	mirrorCamera.quaternion.copy(camera.quaternion);
	mirrorCamera.fov = camera.fov;
	mirrorCamera.aspect = 4.18 / 3.12;
	mirrorCamera.near = 0.05;
	mirrorCamera.far = 90;
...
function renderMirrorDoor() {
	if (!roomMirrorPlane || !portalWindowRuntimeEnabled()) return;
	updateMirrorDoorCamera();
	const previousTarget = renderer.getRenderTarget();
	const previousXrEnabled = renderer.xr.enabled;
	const previousShadowAutoUpdate = renderer.shadowMap.autoUpdate;
	const previousVisible = roomMirrorPlane.visible;
	const previousEntranceVisible = roomEntranceMirrorPlane ? roomEntranceMirrorPlane.visible : false;
	roomMirrorPlane.visible = false;
	if (roomEntranceMirrorPlane) roomEntranceMirrorPlane.visible = false;
	...
	roomMirrorPlane.visible = previousVisible;
	if (roomEntranceMirrorPlane) roomEntranceMirrorPlane.visible = previousEntranceVisible;
}
...
function updateRuntimeDiagnostics(dt) {
	...
	const lines = [
		...
		`ROOM     index=${room.index} souls=${room.depositedSouls}/${room.requiredSouls} door=${room.doorOpen ? "open" : "locked"}`,
		...
		`DOOR     crossing=${doorCrossingState.active ? doorCrossingState.mode : "idle"} dir=${doorCrossingState.direction} cooldown=${room.doorLoopCooldown.toFixed(2)}`,
		`GRAPHICS preset=${graphicsSettings.preset} legacy=${isLegacyGraphicsMode() ? "yes" : "no"} scale=${Math.round(graphicsSettings.renderScale * 100)}% portalWindow=${portalWindowRuntimeEnabled() ? "on" : "off"} particles=${particlesRuntimeEnabled() ? "on" : "off"}`,
...
function updateFrame(time = 0) {
	...
	if (doorCrossingState.active) {
		updateDoorCrossingState(dt);
		updateCamera();
		updateScreenFlash();
		updateTargets();
		updateCrosshair();
