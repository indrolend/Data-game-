...
function enterNextRoomThroughPortal() {
	room.index++;
...
function loopPlayerThroughClosedDoor(direction = "sourceToDestination") {
	...
	player.jumpVel = player.vel.y || player.jumpVel;
	// Keep the chase camera as a normal observer, but if it is already inside the
	// doorway aperture, carry it through the same seam so the door behaves like a
	// connected opening rather than a camera-blocked teleport.
	if (cameraMode.current === CAMERA_MODE_THIRD && isInsideDoorAperture(camera.position, 0.72)) {
		mapSameRoomPortalCrossingPosition(camera.position, portalMappedCameraPosition, direction);
		camera.position.copy(portalMappedCameraPosition);
	}
	room.lastDoorCheckZ = player.pos.z;
...
function tryResolveDoorCrossing() {
	...
	if (!room.doorOpen) {
		if (player.pos.z <= fixedPortal.sourceZ) {
			const delta = fixedPortal.destinationZ - fixedPortal.sourceZ;
			player.pos.z += delta;
			room.lastDoorCheckZ = player.pos.z;
			return true;
		}
		if (player.pos.z >= fixedPortal.destinationZ) {
			const delta = fixedPortal.destinationZ - fixedPortal.sourceZ;
			player.pos.z -= delta;
			room.lastDoorCheckZ = player.pos.z;
			return true;
