...
function getPlayerRoomTileOriginZ() {
	return getRoomTileOriginZ(getRoomTileIndex(player.pos.z));
}
...
function mapSameRoomPortalPosition(sourcePosition, target = new THREE.Vector3()) {
	...
	const tileIndex = getRoomTileIndex(sourcePosition.z);
	const distanceFromDoorInside = THREE.MathUtils.clamp(
		wrapZ(sourcePosition.z) - fixedPortal.sourceZ,
	...
	target.set(
		...
		fixedPortal.destinationZ - distanceFromDoorInside + getRoomTileOriginZ(tileIndex)
	);
	return target;
}
...
function updateRoomVisuals() {
	roomGroup.position.z = getRoomTileOriginZ(getRoomTileIndex(player.pos.z));
	renderGoalSlots();
...
function resolvePlayerObstacleCollisions() {
	...
	let localPlayerZ = getPlayerLocalZ();
	for (const c of roomColliders) {
...
function applyPortalBodyHandoff(targetRoomAdvanced = false) {
	// Compatibility no-op: wrapped-space continuity no longer remaps player/camera.
}
...
function enterNextRoomThroughPortal(direction = "sourceToDestination") {
	...
	// only advance room content ownership.
	room.index++;
...
function tryResolveDoorCrossing(previousPlayerZ = player.pos.z) {
	// Continuous wrapped-space owner: no teleport/remap for closed-door looping.
	const previousTile = getRoomTileIndex(previousPlayerZ);
	const currentTile = getRoomTileIndex(player.pos.z);
	...
	if (room.doorOpen && currentTile < previousTile) {
		resolveDoorCrossingAction("advance", "sourceToDestination");
		return true;
	}
	return false;
}
	...
	for (let i = flowerPowerups.length - 1; i >= 0; i--) {
		...
		if (powerup.continuationMeshes) {
			for (let c = 0; c < powerup.continuationMeshes.length; c++) {
				...
				continuationMesh.position.set(powerup.x, powerup.mesh ? powerup.mesh.position.y : powerup.y, powerup.z + tileOriginZ + ROOM_CONTINUATION_OFFSETS[c]);
...
function keepSoulCoreOutsidePhoneSolid(idx, preferScreenFront = false) {
	...
	targetData[idx + T_Z] = phoneSolidWorld.z - tileOriginZ;
	targetData[idx + T_VX] = 0;
	targetData[idx + T_VY] = 0;
	targetData[idx + T_VZ] = 0;
	return true;
...
function releaseSoulFromScreen(i, idx) {
	...
	const worldZ = targetData[idx + T_Z] + tileOriginZ;
	soulPoint.set(
		targetData[idx + T_X],
		targetData[idx + T_RENDER_Y] || targetData[idx + T_Y],
		worldZ
...
function constrainThirdPersonCamera(desired, lookBase) {
	...
	const tileOriginZ = getRoomTileOriginZ(getRoomTileIndex(lookBase.z));
	const localStartZ = wrapZ(cameraCollisionStart.z);
	const localEndZ = wrapZ(cameraCollisionEnd.z);
	let nearestT = 1;
	for (const c of roomColliders) {
	...
	desired.z = tileOriginZ + THREE.MathUtils.clamp(wrapZ(desired.z), -ROOM_DEPTH / 2 + CAMERA_COLLISION_RADIUS, ROOM_DEPTH / 2 - CAMERA_COLLISION_RADIUS);
...
function updatePlayerPhysics(dt) {
	...
	const crossedPortalDuringMove = tryResolveDoorCrossing(previousPlayerZ);
	clampPlayerToRoomHeight();
...
function updateRuntimeDiagnostics(dt) {
	...
	const lines = [
		...
		`DOOR     crossing=${doorCrossingState.active ? doorCrossingState.mode : "idle"} dir=${doorCrossingState.direction} tile=${getRoomTileIndex(player.pos.z)}`,
